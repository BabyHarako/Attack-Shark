...
const DPI_ORDER = [400, 800, 1200, 1600, 2400, 3200, 5600, 8000, 12000, 42000];
...
function renderDpiPresets(){
  ...
  if(count === 4){
    ...
    // Matches the official app's default 6-gear screenshot exactly:
    // 400, 2400, 3200, 5600, 8000, 42000. Only the first 4 are verified —
    // 8000/42000 will show "not mapped" if clicked, same as everywhere else.
    indices = [0, 4, 5, 6, 7, 9];
  }else{
...
/* Shared send path — used by clicking a gear box AND by the slider/number-box/
   +/- controls, so DPI actually changes on the mouse from either interaction,
   not just the box clicks. Only sends when the target value has real captured
   bytes (400/2400/3200/5600 right now); anything else honestly logs
   "not mapped" instead of guessing, same as the rest of this tool. */
...
el('dpiSlider').addEventListener('change', e => {
  const confirmed = getConfirmedDpiValues();
  const raw = parseInt(e.target.value);
  const nearest = confirmed.reduce((a, b) => Math.abs(b - raw) < Math.abs(a - raw) ? b : a);
  syncDpiDisplay(nearest);
  sendDpiValue(nearest, activeDpiIndex);
});
...
// +/- now jump between the DPI values we actually have real bytes for,
// instead of stepping by a flat 50 — stepping by 50 almost always landed
// on an uncaptured value (correctly refused, but felt "broken"). This way
// every +/- click guarantees a real change on the mouse.
function getConfirmedDpiValues(){
  return Object.keys(PROTOCOL.dpi).map(Number).sort((a,b) => a - b);
}

el('dpiMinusBtn').addEventListener('click', () => {
  const confirmed = getConfirmedDpiValues();
  const current = parseInt(el('dpiNumberBox').value);
  let lower = confirmed.filter(v => v < current);
  const target = lower.length ? lower[lower.length - 1] : confirmed[0];
  syncDpiDisplay(target);
  sendDpiValue(target, activeDpiIndex);
});
el('dpiPlusBtn').addEventListener('click', () => {
  const confirmed = getConfirmedDpiValues();
  const current = parseInt(el('dpiNumberBox').value);
  let higher = confirmed.filter(v => v > current);
  const target = higher.length ? higher[0] : confirmed[confirmed.length - 1];
  syncDpiDisplay(target);
  sendDpiValue(target, activeDpiIndex);
});
