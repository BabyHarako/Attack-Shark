...
el('dpiSlider').addEventListener('change', e => sendDpiValue(parseInt(e.target.value), activeDpiIndex));
...
el('dpiMinusBtn').addEventListener('click', () => {
  const newVal = parseInt(el('dpiNumberBox').value) - 50;
  syncDpiDisplay(newVal);
